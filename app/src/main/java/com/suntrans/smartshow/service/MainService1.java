...
public class MainService1 extends Service {
    ...
    public Socket sixClient = null;
//  public String serverip="61.235.65.160";     //服务器IP
    public String serverip="192.168.1.213";//串口服务器IP
    public String sixIp="192.168.1.213";//第六感IP

//    public int port=8085;    //服务器端口
    public int port=8000;  //服务器端口
    public int sixPort=8000;  //第六感端口

    private String Addr="0000";    //第六感官的地址，从0000到ffff。跟用户名相对应
    private IBinder binder;
    private String SerialNumber;   //手机唯一标识
    private ConnectivityManager mConnectivityManager;
    private NetworkInfo netInfo;
    public final static int SWITCH=2;      //开关代号
    public final static int THREEPHASE=3;  //三相电表代号
    public final static int SIXSENSOR=4;   //第六感代号
...
//        serverip =sharedPreferences.getString("serverip", "61.235.65.160");   //读取服务器ip，若没有则是-1
//        port=Integer.valueOf(sharedPreferences.getString("port", "8778"));
                    ...
                    public void run(){
                        ...
                        try
                        {
                            ...
                            if(dev==SWITCH||dev==SIXSENSOR)
                                  ...
                                  str =toServer + Converts.getMeterCS(bt, 1, bt.length) + "16";  //电表，添加校验和结束符
                            ...
                            {
                                if (client==null){
                                    byte[] tem1 = "连接服务器不成功".getBytes();
                                    RxBus.getInstance().post(tem1);
                                }
                                ...
                                bt = Converts.HexString2Bytes(toServer);
                                if(dev==SWITCH||dev==SIXSENSOR)
                                    str="f1"+toServer+Converts.GetCRC(bt, dev, bt.length)+"0d0a";   //添加校验码和包尾
                                else
                                    str = toServer + Converts.getMeterCS(bt, 1, bt.length) + "16";  //电表，添加校验和结束符
...
//                                    handler1.sendMessage(msg);
     ...
     * 建立第六感连接
     */
    private void connectToSixSensor() {
        new Thread(){      //不能在主线程中访问网络，所以要新建线程
            public void run(){   //新建线程连接服务器，不占用主线程
                try
                {
