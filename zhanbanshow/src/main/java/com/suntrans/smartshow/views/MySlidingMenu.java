...
public class MySlidingMenu extends ViewGroup {
    ...
    private int scrollLeft;    //石建炜更改，用于使侧滑更加容易识别，右滑就认为是显示菜单，左滑就认为是关闭菜单。原来的判断是菜单拖动结束后显示超过一半才显示，反之亦然。现在
                                           //只需要判断滑动结束后srcollLeft的值，>standard就显示，<-standard就关闭，否则恢复原样。
    private int standard=50;   //  滑动的距离超过这个值就更改菜单栏的显示
    ...
    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {   //事件拦截，拦截后就不会向下分发了
        ...
        switch (ev.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(deltaX) > Math.abs(deltaY)&&Math.abs(deltaX)>2){//横向滑动   ，防抖动
                    intercept = true;
                }else{//纵向滑动
                    intercept = false;
                }
...
//                Log.i("Event","事件响应里滑动距离"+String.valueOf(scrollLeft));
                break;

            case MotionEvent.ACTION_UP:
                /*if (getScrollX() < -mMenuWidth / 2){//打开Menu
//                if (getScrollX() < -10){//打开Menu
                ...
                else if(scrollLeft<-standard){   //关闭Menu
                    mScroller.startScroll(getScrollX(), 0, -getScrollX(), 0, 300);
                    isOpen = false;
                    invalidate();
                }
